...
$sql = "SELECT t.id_trabajador, t.apellidos_nombres, t.numero_documento, c.monto_categoria,
               ...
               n.detalle_horarios,
               n.horas_normales_total, n.horas_25_total, n.horas_35_total, n.horas_nocturnas_total,
               n.monto_base_afp, n.monto_afp, n.monto_neto_final,
               n.bono_beta, n.bono_extra_6, n.bono_nocturno
        FROM trabajadores t
...
<style>
  ...
  .time-field { width: 95px !important; border: 1px solid #ced4da; border-radius: 5px; text-align: center; font-weight: 600; color: #2c3e50; }
  ...
  .modo-fijo-activo .lbl-neto { color: #198754; font-size: 1.1em; text-decoration: underline; }
  .modo-fijo-activo .bg-jornal { background-color: #d1e7dd !important; color: #0f5132 !important; }
</style>
    ...
    <div class="seccion-carga">
        <div class="row align-items-start">
            <div class="col-md-6 border-end pe-4">
                ...
                <small class="text-muted d-block mt-1 fst-italic">Se aplicará redondeo híbrido (Entrada: +5min / Salida: -5min).</small>
            </div>
            <div class="col-md-6 ps-4">
                ...
                <div class="busqueda-flotante">
                    <input type="text" id="inputBuscador" class="form-control border-primary-subtle" placeholder="Buscar..." autocomplete="off" onkeyup="filtrarBusqueda(this)">
    ...
    <div id="resumenCarga" class="card mb-4 border-0 shadow-sm d-none">
        <div class="card-header bg-white py-2 d-flex justify-content-between align-items-center">
            <ul class="nav nav-tabs card-header-tabs" id="myTab" role="tablist">
                <li class="nav-item"><button class="nav-link active" id="found-tab" data-bs-toggle="tab" data-bs-target="#found-content" type="button">Coincidentes (<span id="countFound">0</span>)</button></li>
                <li class="nav-item"><button class="nav-link" id="missing-tab" data-bs-toggle="tab" data-bs-target="#missing-content" type="button">No Registrados (<span id="countMissing">0</span>)</button></li>
            </ul>
            <div class="d-flex align-items-center gap-2">
        ...
        <div class="card-body p-0 tab-content" style="max-height: 250px; overflow-y: auto;">
            <div class="tab-pane fade show active" id="found-content" role="tabpanel"><table class="table table-sm table-hover text-center mb-0 small"><tbody id="bodyResumen"></tbody></table></div>
            <div class="tab-pane fade" id="missing-content" role="tabpanel">
                 <div class="p-3 text-center bg-danger bg-opacity-10 mb-2 border-bottom border-danger"><button class="btn btn-sm btn-danger ms-3 shadow-sm" onclick="abrirModalCrearTrabajador()">REGISTRAR RÁPIDO</button></div>
                <table class="table table-sm table-hover text-center mb-0 small"><tbody id="bodyDesconocidos"></tbody></table>
            </div>
        </div>
    </div>
        ...
        <form id="formNomina">
            ...
            <table class="table table-sm mb-0 text-center align-middle">
                ...
                <?php $i=0; foreach($trabajadores_db as $dni => $t): $i++;
                    ...
                    // AF Inicial
                    $af_mensual = $rmv * 0.10; 
                    $af_diario = $af_mensual / 30; 
                    $af_total = $t['tiene_hijos'] ? ($af_diario * $diasBD) : 0;
                ...
                ?>
                    <tr class="fila-nomina <?= $claseOculta ?> <?= $claseNoPlanilla ?>" id="fila-<?= $dni ?>" 
                        ...
                        <td class="text-start ps-3">
                            ...
                            <input type="hidden" name="trab[<?= $i ?>][id_nomina]" value="<?= $t['id_nomina'] ?? 0 ?>">
                            
                            <input type="hidden" name="trab[<?= $i ?>][json_horarios]" class="val-json" value="<?= $jsonDB ?>">
                        </td>
                        
                        <td>
                            <input type="checkbox" class="form-check-input border-secondary chk-fijo" title="Sueldo Fijo" onchange="toggleFijo(this)">
                        </td>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.fila-nomina').forEach(tr => { inyectarFilaEspecifica(tr.dataset.dni); });
    pintarRangoActivo();
});
...
function toggleFijo(chk) {
    ...
    tr.querySelector('.val-json').value = JSON.stringify(obj);
    inyectarFilaEspecifica(dni);
}

function recargarFiltros(){ 
    const url = new URL(window.location.href);
    url.searchParams.set('mes', document.getElementById('mes_pago_sel').value);
    url.searchParams.set('periodo', document.getElementById('periodo_pago_sel').value);
    url.searchParams.set('fi', document.getElementById('periodo_inicio').value);
    url.searchParams.set('ff', document.getElementById('periodo_fin').value);
...
function toggleVerTodos() {
    ...
    document.querySelectorAll('.fila-nomina').forEach(tr => {
        if(ver) tr.classList.remove('fila-oculta'); else if((parseInt(tr.querySelector('.inp-dias').value)||0) === 0) tr.classList.add('fila-oculta');
    });
    recalcTotalGeneral();
}

function procesarArchivoExcel() {
    const rango = getRangoActivo(); if(!rango) return alert("⚠️ Defina y APLIQUE rango de fechas.");
    const file = document.getElementById('inputExcel').files[0]; if(!file) return alert("⚠️ Seleccione Excel.");
    ...
    reader.onload = function(e) {
        try {
            ...
            let start = 0; for(let i=0;i<json.length;i++){ if(json[i][0] && String(json[i][0]).length>=8){ start=i; break; } }
            for(let i=start; i<json.length; i++){
                ...
                for(let k=0;k<raw.length;k++){
                    ...
                    if(s > 0){
                        let tM = Math.floor(s / 60); let rM = (k % 2 === 0) ? Math.ceil(tM / 5) * 5 : Math.floor(tM / 5) * 5;
...
function parseFechaFlexible(v) {
    if (!v) return null; if (typeof v === 'number') return new Date(Math.round((v - 25569) * 86400 * 1000)).toISOString().split('T')[0];
...
function renderReporte() {
    const b1=document.getElementById('bodyResumen'), b2=document.getElementById('bodyDesconocidos'); b1.innerHTML=""; b2.innerHTML="";
    let c1=0, c2=0;
...
function inyectarFilaEspecifica(dni){
    ...
    for(let f in obj){
        ...
        for(let i=0;i<raw.length;i+=2){ const t1=parseTime(raw[i]), t2=parseTime(raw[i+1]); if(t2>t1){ s+=(t2-t1); tNoct+=calculateNightSeconds(t1,t2); } }
        s=Math.round(s/300)*300; let h=s/3600; tN += Math.min(h,8); let ex = Math.max(0,h-8); t25 += Math.min(ex,2); t35 += Math.max(0,ex-2);
    }
    ...
    calcularFila(tr, tN, t25, t35, tNoct/3600);
}

function calcularFila(tr, hN=0, h25=0, h35=0, hNoct=0) {
    const isFijo = tr.querySelector('.chk-fijo').checked; const rb = parseFloat(tr.dataset.rb) || 0; const ps = parseFloat(tr.dataset.seg) || 0; const hijos = parseInt(tr.dataset.hijos) || 0; const enPlanilla = tr.dataset.planilla; 
    ...
    if(!isFijo) {
        ...
        for(let f in diasObj){
            if(f==='es_fijo' || f==='backup_data') continue;
            let s=0; const raw = (diasObj[f].raw || "").split(','); for(let i=0;i<raw.length-1;i+=2){ let t1=parseTime(raw[i]), t2=parseTime(raw[i+1]); if(t2>t1) s+=(t2-t1); }
            if(Math.round(s/300)*300 > 14400) cB++;
        }
    ...
    tr.querySelector(".lbl-rb").innerText = rbh.toFixed(4); tr.querySelector(".lbl-grati").innerText = gh_hora.toFixed(4); tr.querySelector(".lbl-cts").innerText = ct_hora.toFixed(4);
    ...
    tr.querySelector(".lbl-descto").innerText = afp.toFixed(2); tr.querySelector(".lbl-neto").innerText = neto.toFixed(2);
    tr.querySelector(".val-afp").value = afp.toFixed(2); tr.querySelector(".val-neto").value = neto.toFixed(2);
    tr.querySelector(".val-beta").value = beta.toFixed(2); tr.querySelector(".val-bono6").value = bet6.toFixed(2);
    tr.querySelector(".val-bnoct").value = (isFijo ? 0 : (hNoct * (jornal_hora_total * 0.35))).toFixed(2);
    recalcTotalGeneral();
}

function recalcTotalGeneral(){ let t=0; document.querySelectorAll('.fila-nomina:not(.fila-oculta) .lbl-neto').forEach(e => { t += parseFloat(e.innerText.replace(/,/g, '')) || 0; }); document.getElementById('totalGeneral').innerText = t.toLocaleString('en-US', {minimumFractionDigits: 2}); }
...
function genInp(v1, v2, p){ const b = p ? '' : `<button class="btn btn-sm text-danger ms-1 border-0" onclick="removerTramo(this)"><i class="bi bi-x-circle-fill"></i></button>`; return `<div class="tramo-input d-flex align-items-center gap-1"><input type="time" class="time-field form-control form-control-sm" value="${v1}" onchange="recalcM(this)"><i class="bi bi-arrow-right small text-muted"></i><input type="time" class="time-field form-control form-control-sm" value="${v2}" onchange="recalcM(this)">${b}</div>`; }
...
function guardarCambiosModal(){ const f = document.getElementById('fila-'+dniActivo); let n = {}; try{ n = JSON.parse(f.querySelector('.val-json').value || '{}'); } catch(e){} const r = getRangoActivo(); for(let k in n) { if(dentroDeRango(k, r)) delete n[k]; } document.querySelectorAll('#bodyModal tr').forEach(tr => { const fecha = tr.querySelector('.f-txt')?.innerText; if(fecha && dentroDeRango(fecha, r)){ const t = []; tr.querySelectorAll('input[type="time"]').forEach(i => { if(i.value){ let v=i.value; if(v.length===5)v+=":00"; t.push(v); } }); if(t.length > 0 && t.length % 2 === 0) n[fecha] = { raw: t.join(',') }; } }); f.querySelector('.val-json').value = JSON.stringify(n); inyectarFilaEspecifica(dniActivo); bootstrap.Modal.getInstance(document.getElementById('modalAsistencia')).hide(); }
...
function recalcM(el){ if(!el) return; const tr = el.closest('tr'); let s = 0; tr.querySelectorAll('.tramo-input').forEach(d => { const i = d.querySelectorAll('input'); if(i.length == 2){ const t1 = parseTime(i[0].value), t2 = parseTime(i[1].value); if(t2 > t1) s += (t2 - t1); } }); let h = Math.round(s/300)*300 / 3600; tr.querySelector('.c-n').innerText = decimalToTime(Math.min(h, 8)); tr.querySelector('.c-25').innerText = decimalToTime(Math.min(Math.max(0, h-8), 2)); tr.querySelector('.c-35').innerText = decimalToTime(Math.max(0, h-10)); sumM(); }
